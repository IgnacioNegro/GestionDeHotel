...
namespace ObligatorioProg
{
    public class GestionHabitacion
    {
        ...
        /*        public List<Habitacion> listaHabitaciones = new List<Habitacion>();
                ...
                public List<Habitacion> ConsultarHabitacionesDisponibles(DateTime fechaInicio, DateTime fechaFin)
                {
                    var habitacionesDisponibles = listaHabitaciones.Where(h => !h.FechasReservadas.Any(fecha => fecha >= fechaInicio && fecha <= fechaFin)).ToList();
                    ...
                    if (habitacionesDisponibles.Count > 0)
                    {
                        ...
                        {
                            Console.WriteLine($"Número: {habitacion.NumeroHabitacion}, Tipo: {habitacion.ToString}, Precio: {habitacion.Precio}");
                        }
                    }
                    else
                    {
